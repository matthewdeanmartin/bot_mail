...
PACKAGE := bot_mail
PYTHON_TARGETS := bot_mail tests
PYLINT_MAIN_TARGETS := bot_mail
...
ABOUT_FILE := bot_mail/__about__.py
...
help:
	...
	@echo "  docs-check-links       linkcheckMarkdown README.md || true"
...
dont-be-lazy:
	...
	@$(UV) dont_be_lazy --root . --no-color scan bot_mail --no-config-suppressions || true

.PHONY: pydoc-docs
pydoc-docs:
	@$(UV) pydoc_fork bot_mail -o ./pydoc/
